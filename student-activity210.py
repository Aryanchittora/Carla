...
def number_of_vehicle():
    ...
    transform = dropped_vehicle.get_transform()
    print('Location Of Each Bot Vehicles - ', transform)

    if len(all_vehicles) > 1:
        distance = lambda data: math.sqrt(
            (data.x - transform.location.x) ** 2 + (data.y - transform.location.y) ** 2 + (
                        data.z - transform.location.z) ** 2)
