...
def car_control():
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.52, steer=-1.0, gear=0))
    time.sleep(5)

    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, gear=0))
    time.sleep(6)

    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, steer=-0.15, gear=0))
    time.sleep(2)
...
try:
    ...
    def _on_collision(data):
        ...
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
        # dropped_vehicle.apply_control(carla.VehicleControl(reverse=True, gear=-1, throttle=0.3))
        # time.sleep(5)
        car_control()
