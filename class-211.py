        ...
        for distance_of_car, vehicle in sorted_vehicles:
            ...
            for distance_in_meter in final_vehicle_result:
                if distance_in_meter['distance'] > 3 and distance_in_meter['distance'] < 6:
                    ...
                    time.sleep(2)
                    dropped_vehicle.apply_control(carla.VehicleControl(reverse=True, throttle=0.4))
                    car_control(collision=True)
...
def car_control(collision):
    if collision == True:
        ...
        dropped_vehicle.apply_control(carla.VehicleControl(steer=-0.9, throttle=0.5))
        time.sleep(1.8)
