...
def generate_lidar_blueprint(blueprint):
    lidar = blueprint.find('sensor.lidar.ray_cast_semantic')
    lidar.set_attribute('channels', str(64))
    lidar.set_attribute('points_per_second', str(56000))
    lidar.set_attribute('range', str(100))
    lidar.set_attribute('rotation_frequency', str(40))
    return lidar
...
def semantic_lidar_data(point_cloud):
    ...
    for x in point_cloud:
        pos = value_list.index(x.object_tag)
        dist = math.sqrt((x.point.x ** 2) + (x.point.y ** 2) + (x.point.z ** 2))
        distance_name_data["distance"] = dist
        distance_name_data["name"] = key_list[pos]
