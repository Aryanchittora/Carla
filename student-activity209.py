...
def car_control():
    doors = carla.VehicleDoor.All
    
    dropped_vehicle.open_door(self, doors)
...
try:
    ...
    def _on_collision(data):
        ...
        actor_name = actorName(data.other_actor)
        print('Collided With ', actor_name)
        collision_record = data.normal_impulse
        intensity = math.sqrt(collision_record.x * 2 + collision_record.y * 2 + collision_record.z * 2)
        print("Collided With ", intensity, 'Intensity')
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
