...
def Bike_control():
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.6))
    time.sleep(12.5)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, steer=-0.3))
    time.sleep(2.8)
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        for i in object_id:
            if distance_name_data['name'] == 'Buildings' and distance_name_data["distance"] > 3 and distance_name_data["distance"] < 12:
