...
def car_control(check):
    if check == True:
        dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.7, steer=-0.5))
        time.sleep(4.7)
...
try:
    ...
    def _Radar_callback(radar_data):
        ...
        for detection in radar_data:
            ...
            if distance_name_data["distance"] > 3 and distance_name_data["distance"] < 6.59:
                ...
                dropped_vehicle.apply_control(carla.VehicleControl(reverse=True, throttle=0.3))
                time.sleep(4)
                dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
                time.sleep(1)
