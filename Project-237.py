...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        car_data = ([[throttle, distance_name_data["distance"]]])
        car_predict = model.predict(car_data)
        prediction = int(car_predict[0])
        #Getting prediction
        if prediction == 1:
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.1, reverse=True))
