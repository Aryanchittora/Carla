...
def generate_radar_blueprint(blueprint_library):
    radar = blueprint_library.filter('sensor.other.radar')[0]
    radar.set_attribute('horizontal_fov', str(40))
    radar.set_attribute('vertical_fov', str(30))
    radar.set_attribute('points_per_second', str(15000))
...
def _Radar_callback(data):
    distance_name_data = {}

    for x in data:
        distance_name_data['distance'] = x.depth
        ...
        if distance_name_data['distance'] > 5 and distance_name_data['distance'] < 7:
            ...
            if distance_name_data['distance'] > 3 and distance_name_data['distance'] < 5:
                print("Braking")
                dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
                print('Changed Name & Distance -', distance_name_data)
