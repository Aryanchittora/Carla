...
def lidar_data(point_cloud, point_cloud_buffer):
    matrix = np.copy(np.frombuffer(point_cloud.raw_data, dtype=np.dtype('f4')))
    reshape = np.reshape(matrix, int(matrix.shape[0] / 4), 4)
    intensity = reshape[:, -1]
    lidar_points = reshape[:, :-1]
    lidar_points[:, :1] = lidar_points[:, :1]
    point_cloud_buffer['pts'] = lidar_points
    point_cloud_buffer['intensity'] = intensity

def generate_lidar(blueprint_library):
...
try:
    ...
    lidar_sensor = generate_lidar(get_blueprint_of_world)
