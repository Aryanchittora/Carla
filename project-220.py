...
def semantic_lidar_data1(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        if distance_name_data["name"] == 'Vehicles' and distance_name_data["distance"] > 3 and distance_name_data["distance"] < 10:
...
try:
    ...
    police_spawn_point = world.get_map().get_spawn_points()[15]
    police_car = world.spawn_actor(police_car_blueprint, police_spawn_point)
    ...
    simulator_camera_location_rotation = carla.Transform(police_spawn_point.location, police_spawn_point.rotation)
    simulator_camera_location_rotation.location += police_spawn_point.get_forward_vector() * 30
